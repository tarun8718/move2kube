...
func TestPodman(t *testing.T) {
	...
	t.Run("Check for InspectImage functionality ", func(t *testing.T) {
		...
		for _, val := range outputInspect {
			if val.RepoTags[0] != "docker.io/library/alpine:latest" {
				t.Fatalf("Ispect Repo Tag Mismatch Should be - alpine:latest got - %v", val.RepoTags[0])
			}
		}
	})
}
